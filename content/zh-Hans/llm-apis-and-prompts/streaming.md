...
为什么流式聊天里第二轮往往比第一轮快得多？因为服务端把两次调用之间共享前缀（system prompt、之前的轮次、检索到的上下文）的 KV 状态缓存住了，只需要给新增 token 做 prefill 就行。**第 7 章（KV Cache）**讲机制。**第 8 章（推理并发）**讲一个服务栈如何在大量并发用户之间管理这份缓存。
