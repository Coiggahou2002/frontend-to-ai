...
姊妹章节（[第 10 章](../post-training)）讲的是后训练的*理论*——SFT、DPO、RLHF、GRPO——停留在"这些算法为什么存在"这个层面。本章则一直坐在工程师的位子上：加载基座模型、挂上 LoRA adapter、按 chat 格式整理数据、在单张 GPU 上跑训练、评估、上线。锁定的样例是 **Qwen-3B 上跑 QLoRA**，能塞进免费的 Colab T4。同样的代码搬到更大的 GPU 上跑更大的模型也照样能用。
...
关于本章范围的一句话：本章是工程的。[第 10 章](../post-training) 是理论搭档——读那一章是为了搞清楚 *为什么* SFT 有效、DPO 和 GRPO 是什么、人类偏好数据是怎么走进训练流程的。这些东西本章不会再推一遍。
