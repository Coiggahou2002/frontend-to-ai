...
- **单轮平均成本。** 单次迭代 token 花费，按对话记录长度归一化。出现尖峰意味着 prefix caching（[第 7 章](../kv-cache)）回归了，或者对话记录炸了。
...
Agent 评估是 LLM 评估的一个特例。同样的纪律（标注集、裁判校准、回归跟踪、用分布层级指标替代相等断言）适用于 chat completion、RAG、分类器，以及一切产生非确定性输出的东西。**第 11 章（评估与可观测性）**会深入更多：golden set、A/B 测试、在线 vs. 离线评估、漂移检测，以及怎么在真实工程团队里把这一切落地运营。

---

接下来，自部署任何这些东西的硬件现实——[第 5 章](../gpu-and-model-sizing)。
