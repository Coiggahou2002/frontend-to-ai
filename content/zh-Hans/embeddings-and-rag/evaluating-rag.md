...
本节里的所有内容都是某种更通用纪律的实例。**第 11 章（评估与可观测性）**把 LLM 评估当作一等话题处理——输出的分布、judge 模型校准、回归追踪、在线 vs 离线评估。RAG 是最容易起步的地方，因为失败面定义清晰、指标具体；一旦你有了检索的 golden set，离任何东西都有 golden set 也就走完了 80%。
