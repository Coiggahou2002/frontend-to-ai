...
**2. 评估集永远不进训练数据。**永远不。如果你做微调（[第 9 章](../fine-tuning)），你的 held-out 评估集必须和训练数据**密码学层面**地隔离——不同文件、做哈希校验、在训练流水线里作为显式的 deny-list 写入。在你的测试集上训练会让你的分数虚高，模型看起来比实际好，你以为是个改进，实际上你把更差的模型推到了生产。每个团队都要踩一次这个坑。别当那个团队。
