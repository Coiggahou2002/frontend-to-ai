...
- 微调需要一个 held-out 的 regression set，不然你根本不知道这次微调到底是有帮助还是搞砸了（[第 9 章](../fine-tuning)）。
...
本章后面假设的读者，是读完了第 10 章的那个人：能调用模型、能搭 RAG、能上线 agent、能微调、能自部署。他们还做不到的，是**知道这些东西到底有没有真的在工作**。这一章就是为了解决这件事。
